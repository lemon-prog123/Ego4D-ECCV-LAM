...
class MagLinear(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features, scale=64, easy_margin=True,args=None):
        ...
        #self.bias=Parameter(torch.Tensor(torch.zeros(out_features)))
        
        #if args.focal:
        #    self.bias.data=torch.FloatTensor([-4.59,4.59])
    ...
    def forward(self, x, m, l_a, u_a):
        ...
        cos_theta = torch.mm(F.normalize(x), weight_norm)#+self.bias
        cos_theta = cos_theta.clamp(-1, 1)
        sin_theta = torch.sqrt(1.0 - torch.pow(cos_theta, 2))

        cos_theta_m = cos_theta * cos_m - sin_theta * sin_m

        if self.easy_margin:#cos_theta>0
...
class MagLoss(torch.nn.Module):
    ...
    def __init__(self, l_a, u_a, l_margin, u_margin, scale=64.0,weight=None):
    ...
    def forward(self, input, target, x_norm):
        ...
        #print(output,target)
        loss = F.cross_entropy(output, target, weight=self.weight,reduction='mean')
        #print(loss)
